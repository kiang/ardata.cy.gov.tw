...
foreach (glob(dirname(__DIR__) . '/data/individual/account/113年立法委員選舉/*/*_1.zip') as $zipFile) {
...
foreach (glob(dirname(__DIR__) . '/data/individual/account/109年立法委員選舉/*/*_1.zip') as $zipFile) {
    $pathParts = explode('/', $zipFile);
    $reportPath = $dupPath . '/' . $pathParts[8];
    if (!file_exists($reportPath)) {
        mkdir($reportPath, 0777, true);
